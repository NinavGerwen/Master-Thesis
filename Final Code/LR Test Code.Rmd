...
for(i in 1:100){
  data <- data.gen(300, 5, "2PL")
  
  l_0 <- ltm(data ~ z1)$log.Lik
  
  p_values[i] <- mirt.diff.test(l_0 = l_0, dataset = data, k = 5)
}
...
for(i in 1:300){
  data <- data.gen(300, 10, model = "3PL")
  
  model <- ltm(data ~ z1, control = list(iter.em = 100, 
                                         iter.qN = 1000,
                                         GHk = 40))
}
) ## time: 84.856, 3 errors

set.seed(1248)
system.time(
for(i in 1:300){
  data <- data.gen(300, 10, model = "2PL")
  
  model <- ltm(data ~ z1, control = list(iter.em = 300, 
                                         iter.qN = 1000,
                                         GHk = 100))
}
) ## time: 374.402, 5 errors
...
for(i in 1:300){
  ...
  model <- ltm(data ~ z1, control = list(iter.em = 50, 
                                         iter.qN = 500,
                                         GHk = 30))
}
) ## time: 54.091, 4 errors
...
system.time(
for(i in 1:300){
ltm(data.gen(200, 5, "2PL") ~ z1, IRT.param = TRUE)
}
) ## .018 time, 300 loops: 5.906

set.seed(1248)
system.time(

for(i in 1:300){
ltm(data.gen(200, 5, "2PL") ~ z1, IRT.param = TRUE, start.val = start_vals)
}
) ## .013 time, 300 loops: 4.493

set.seed(1248)
system.time(
for(i in 1:300){
ltm(data.gen(200, 5, "3PL") ~ z1, IRT.param = TRUE, start.val = start_vals)
}) ## 5.712 time, some errors

set.seed(1248)
system.time(
for(i in 1:300){
ltm(data.gen(200, 5, "3PL") ~ z1, IRT.param = TRUE)
}) ## 7.105, same amount of errors
```
...
Hieronder: data generatie code voor equal gammas
```{r}
