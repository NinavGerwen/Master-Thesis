...
## HIERVOOR THETA NAAR -0.5 VERANDERD, NIET MEER -1, VERANDER TERUG NINA
...
PL3_gamma_diff <- data.frame(test_length = NA, sample_size = NA, G = NA, MG.LR = NA)
  ...
  for(b in levels(sample_size)){
    ...
    for(d in levels(n_groups)){
      ...
      start.vals <- start.values(temp_k)
        ...
        PL3_gamma_diff <- rbind(PL3_gamma_diff, c(a, b, d, mean(prop_1)))
      }
...
PL3_gamma_diff <- PL3_gamma_diff %>%
  pivot_wider(., names_from = G, values_from = MG.LR) %>%
  .[-1, -3]

rownames(PL3_gamma_diff) <- NULL
colnames(PL3_gamma_diff) <- c("test_length", "sample_size", "LR2", "LR3", "LR4")
...
### TEST with less extreme diff

```{r}
set.seed(1248)

less_extreme_test <- data.frame(test_length = NA, sample_size = NA, G = NA, MG.LR = NA)
    ...
    for(d in levels(n_groups)){
      ...
      start.vals <- start.values(temp_k)
        ...
        ## current condition
          ...
          temp_data <- less.extreme.data.gen(n = temp_n, k = temp_k, g = temp_g, 
                                           method = "param", model = "2PL")
        ...
        less_extreme_test <- rbind(less_extreme_test, c(a, b, d, mean(prop_1)))
      }
...
less_extreme_test <- less_extreme_test %>% 
  pivot_wider(., names_from = G, values_from = MG.LR) %>%
  .[-1, -3]

rownames(less_extreme_test) <- NULL
colnames(less_extreme_test) <- c("test_length", "sample_size", "LR2", "LR3", "LR4")
