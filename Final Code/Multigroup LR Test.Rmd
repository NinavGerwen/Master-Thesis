...
alpha_results_two <- alpha_results_two[-1 ,]
  ...
  for(b in levels(sample_size)){
    ...
    for(d in levels(n_groups)){
      ...
      start.vals <- start.values(temp_k)
        ...
        power_results_two <- rbind(power_results_two, c(a, as.numeric(as.character(b)) * 4, d, mean(prop_1)))
      }
...
power_results_two <- power_results_two[-1 ,]
...
library(magrittr)
library(ltm)
test <- multigroup.data.gen(n = 100, k = 20, g = 2, method = "theta", model = "2PL")

test <- test %>% as.data.frame(.)

colnames(test) <- c(1:20, "split")

split_set <- test %>% subset(., split == 2, -split)

ltm(split_set ~ z1, IRT.param = TRUE)
```
