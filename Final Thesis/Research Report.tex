...
The current study investigated existing and new possible measures of model fit in the context of IRT to test for model misspecification and group differences. Through a simulation study, we found out that the proposed goodness-of-fit test - the MG LR test - requires sample sizes larger than 1000 in order to properly follow a $\chi^2$ distribution. 

Furthermore, the MG LR test was not able to detect any model misspecification when groups are based on random assignment. Instead, the MG LR test seemed robuust against model misspecification. A follow up simulation study showed that the test is very effective at detecting group differences. Taken together, it means that the MG LR test is a useful addition to the multigroup analysis literature, where the test can test for group differences whilst ignoring model specification. 

Investigating multiple fit indices, we ... that the TLI and CFI work very well. However, against the expectations of Cai (2021), who theorized that their version of the TLI would not depend heavily on model size (i.e., test length), we found evidence that the TLI (and CFI) lower by large amounts as test length increases. Although this could be partly explained by the differences in calculations and choice of null model, it remains an important finding. It would entail that model size should be taken into account when trying to find rules of thumb for use of the TLI and CFI in IRT.

Furthermore, investigating model fit between the 2PL and 3PL in IRT can not be done through methods that explore the difference between expected and observed scorepatterns. 
