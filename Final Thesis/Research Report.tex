...
detect model misfit between the 2PL and 3PL. % verander deze shit
...
to detect misfit between the 2PL and 3PL % hier ook
...
the 2- and 3PL model %inspiratie?
...
to detect model misspecification between the 2PL and 3PL % hier is hij weer
...
model misspecification when working with randomised groups. % hier ook?

However, this means the test is insensitive to violations of model specification. % in testing?
...
model fit decisions between the 2- and 3PL in IRT.  % Hmmm?
...
detect model fit between the 2PL and 3PL in IRT. % yikes haha
...
under different conditions under existing group differences. % twee keer under... I don't like
...
% hier duidelijker maken dat DATA generatie anders is, en dat 'of interest is a power difference between the situation where the right and wrong model is fitted
...
any model misspecification between the 2PL and 3PL when groups were based on random assignment. % same old
...
were also not able to detect misspecification between the 2PL and 3PL at any reasonable rate. % same old

\\
\indent Contrary to these results, we did find that the TLI and CFI were very sensitive 

in detecting misfit between the 2PL and 3PL in one-dimensional IRT. % twee fouten, 2PL 3PL en one-dimen
...
% hier moet het over measurement invariance gaan, laat DIF weg
...
unidimensional dichotomous IRT. % anders. 
