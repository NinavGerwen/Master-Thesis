...
-- ERGENS NOG PRATEN OVER MEASUREMENT INVARIANCE?? Group differences? momenteel komt dit namelijk uit het niets
...
where $L_0$ is the likelihood of the chosen model for the whole dataset and $L_j$ is the likelihood of the chosen model for group $j$. Group $j$ can either be gained by randomly assigning observations to $g$ groups or it could be $g$ existing groups. Wilks' theorem also applies to this likelihood statistic. Because all three aforementioned tests asymptotically follow a $\chi^2$ distribution, they can be used for Null Hypothesis Significance Testing. \\
...
\begin{equation}
\text{ICFI} = \frac{2 \cdot \sum_{\boldsymbol{x}} O_{\boldsymbol{x}} \cdot \sum_{\boldsymbol{x}} E_{\boldsymbol{x}}}{  \sum_{\boldsymbol{x}} \{ O_{\boldsymbol{x}}\}^2 +  \sum_{\boldsymbol{x}} \{ E_{\boldsymbol{x}} \}^2 },
\end{equation}
