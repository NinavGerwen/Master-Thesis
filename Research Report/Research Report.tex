...
The $\chi^2$-difference test instead is difficult to use for both the two- (2PL) and three-parameter (3PL) model. This is because the 3PL and generalisations of the model tend tend to suffer from consistency and estimation issues (Barton \& Lord, 1981; Loken \& Rulison, 2010). 
...
In order to create a goodness-of-fit test to use for the 2PL and 3PL in IRT and to better understand the possible uses of the CFI and TLI in an IRT setting, the present study answers the following three research questions through a simulation study:
\begin{enumerate}
...
-- misschien hier nog toevoegen wat de m.l.e. is van $\beta_{i}$ -- 

Furthermore, to estimate the indices, a saturated model is also required. In a saturated model there are as many parameters as data points, which leads to a perfect fit. We obtained a saturated model with the following formula:
